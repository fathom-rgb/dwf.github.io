...
作品统一维护在 `data/products.json`。新增一条产品数据即可自动进入完整目录，分类与数量自动更新；只有 `featured: true` 的产品进入精选区，按数据顺序最多展示 3 件。当前精选为人物之间与大学文字模拟器。完整目录始终包含全部作品，默认不折叠。
...
- **精选与目录**：人物之间、大学文字模拟器、小账本、个人网站。精选显示真实产品截图，全部作品提供名称、一句话价值、类型、状态与入口，并支持即时分类筛选。
...
采用纸白、墨色和朱砂红配色，以书刊式标题、留白、个人图案与真实项目画面建立视觉层次。首页默认深墨蓝主题，支持切换浅色。首屏作品星图使用原生 WebGL 展示带光照的产品球体、空间轨道和透视旋转；作品数据直接生成星图入口，每组最多 6 件，其余通过分组浏览。

个人主标识采用深靛蓝与陶朱红的双鱼图案，用于页头与浏览器图标。06-A 抱书小法师保留在首屏个人介绍旁，简历与分享预览使用双鱼标识。两款图案位于 `assets/identity/`。
...
| `assets/constellation.css`、`assets/constellation.js` | 作品星图、原生 WebGL 渲染与静态降级 |
...
- 首次绘制前恢复亮暗偏好；首页未手动选择时使用深色主题。
- 星图支持拖动、键盘访问、方向按钮和暂停；离开屏幕或隐藏标签页时停止渲染，减少动态效果时静止。不支持 WebGL 时保留静态入口。
- 手机导航使用原生 `dialog`，支持 Escape、焦点管理与背景滚动锁定。
- 导航链接关闭菜单后将焦点交给对应章节；切回桌面宽度时收起菜单。
- 两件精选产品与全部作品直接展示；分类筛选只作用于完整目录，作品笔记使用原生 `details`。
