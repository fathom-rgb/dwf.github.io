...
const {renderProducts, renderHeroIndex, validate, root} = require('./products.cjs');
...
const index = renderHeroIndex([...products, mock]);
assert.equal((index.match(/class="index-title"/g) || []).length, 4);
assert(index.includes('查看全部 5 件作品'));
for (const p of products.slice(0,4)) assert(index.includes(`href="${p.href}"`));
