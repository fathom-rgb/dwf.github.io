...
采用纸白、墨色和朱砂红配色，以书刊式标题、留白、个人图案与真实项目画面建立视觉层次。深色主题使用墨绿背景与暖色重点。移除粒子、打字机和技能进度条，让内容直接可见。

个人主标识采用深靛蓝与陶朱红的双鱼图案：双眼为圆形，右侧轮廓保持流畅，用于页头与浏览器图标。06-A 抱书小法师作为首页主视觉，简历与分享预览使用双鱼标识。两款图案均依据本人选定的方案制作，位于 `assets/identity/`；网页使用压缩后的图片，并为小尺寸图标单独导出 PNG。
